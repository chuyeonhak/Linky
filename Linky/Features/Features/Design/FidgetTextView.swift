...
extension FidgetTextView {
    ...
    private func calculateForegroundColor(for index: Int) -> Color {
        guard let selectedIndex = selectedCharacterIndex else { return Color(UIColor.code2 ?? .white) }

        let distance = CGFloat(abs(selectedIndex - index))

        if distance == 0 {
            return Color(UIColor.code3 ?? .white)
        } else if distance == 1 {
            return Color(UIColor.code4 ?? .white)
        } else {
            return Color(UIColor.code5 ?? .white)
        }
    }
}
